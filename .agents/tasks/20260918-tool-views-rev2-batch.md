...
## 定向复审结果与 N1 回归（2026-09-18/19）

复审 agent 中途静默停止过一次（见上），重启后完成：commit `f8aef00`，复审记录 427 行。

**复审结论：需修复后合入**，唯一阻塞项 **N1**。

| 修复 | 复审判定 |
| --- | --- |
| F1 红色不再泄漏到成功项 | **部分成立，但引入回归（blocking）**：泄漏确实消失，但红色也彻底不生效 |
| F2 表述改为「窗口高 16 行」 | 基本成立；代码注释残留 3 处（N2） |
| F4 通用降级详情纳入窗口 | **成立** |
| F5 flaky 断言收窄 | **成立**，且有 base 对照的强证据（§2.4） |

**N1（blocking）根因**：`.tool-result-error pre` 与 `.tool-detail pre` 特异性都是 (0,1,1)，后者在源码中更靠后 → 后出现者胜 → 失败正文实际是灰色。修复前 `.tool-error .tool-detail pre` 为 (0,2,1) 稳胜，红色是真生效的（代价是泄漏）。即“泄漏没了，红色也没了”。**测试抓不到**（jsdom 不做级联；现行 `styles.test.ts` 只断言“源码里有这条规则”）。

**其他 findings**：N2（low，3 处代码注释残留「可见 16 行」）；N3（info，F4 后仅剩一处有意的横向滚动）；N4（low，**不阻塞**：剩余 flake 是既有 `ChatAttachments` 5s 超时，在 base 与交付分支以同概率出现 —— 这也解释了 Integrator 在重负载下看到的两次未定位失败）；N5（info，新测试只能锁规则名、锁不住级联，正是它放行 N1）；N6（info，`lineCountNote` 阈值保守会漏提示）。

## 第三轮修复派发（2026-09-18/19）

退回 Worker `herzi_viewrev2`：

1. **N1 必修**：三种最小改法任选（红色规则移到 `.tool-detail pre` 之后 / 提高特异性为 `.tool-detail .tool-result-error pre` / 用继承），要求同时满足「红色只在失败项生效」且「不被灰色规则覆盖」。
2. **N5 一并修**：把断言升级为**能算级联胜者**的形式（解析 styles.css 比较（特异性, 顺序）），降低特异性或移回原位时必须 FAIL。
3. **N2 的 3 处代码注释**改为「窗口高 16 个代码行」；`docs/` 由 Integrator 处理。
4. 不需要动：N3、N4、N6。
