...
| [`tool-call-detail-ui-plan.md`](./tool-call-detail-ui-plan.md) | 已实施（P0–P2.5 + Rev.2）、已合入本地 `main`；待浏览器验收与最终批准 | 工具调用**展开区**的按工具定制视图（bash/read/edit+write/ffgrep+fffind/web_search/web_fetch，第二层 todo/ask_user_question）、`details` 白名单投影、真实数据核对；§11 为 Rev.2（固定 16 行窗口 + shiki 高亮 + Memoh 视觉） |
...
- 2026-09-17 已完成工具调用专属展示方案（未实施）：核对 207 个本机 session、约 9,550 次 `toolResult` 后确认 `edit.details.diff`（454/481）、`ffgrep.details.totalMatched`（112/112）、`todo.details.tasks`（1019/1019）、`ask_user_question.details.answers`（71/71）可在服务端白名单投影；`read.details.truncation`（15/980）与 `bash.details.truncation`（31/5192）基本不可用，需靠 args 推导与尾部摘要解析。方案覆盖 bash / read / edit+write / ffgrep+fffind / web_search / web_fetch 六类展开视图（第二层 `todo` / `ask_user_question` 数据同样完备，列为建议一并做），折叠行与分组规则不变。§9 的四项实施前核对已完成：`ffgrep` 的 `:`/`-` 语义证实为匹配行/上下文行；`edit.details.patch` 是标准 unified diff；`bash.fullOutputPath` 全部位于系统临时目录而非工作目录（原策略会 100% 拒绝，已修正）；`write.details` 实测为空对象。详见 [`tool-call-detail-ui-plan.md`](./tool-call-detail-ui-plan.md)。
- 2026-09-18 工具调用专属展开区已实现并通过**两轮独立 review**：10 个工具 → 8 个视图（`read`/`write` 共用 `CodeView`，`ffgrep`/`fffind` 共用 `MatchListView`）；协议新增受限 `display` 白名单投影，`result` 语义、折叠行、分组与 `Worked for` 结构均未变。第一轮 6 条 finding（2 medium / 2 low / 2 info）已闭合，第二轮结论「可合入」；集成时由 Integrator 修掉 F6-low（`web_search` 编号重复时的 React key 冲突，含回退验证）。集成态 `typecheck` / 264 tests / shuffle / `build` 全部 PASS；真实浏览器与真实 Pane 验收 `NOT RUN`；P3（宿主文件读取）未实现。已合入本地 `main`（`6167cc6`），**未 push，等最终批准**。详见 `.agents/tasks/20260917-tool-call-detail-ui-batch.md`。
- 2026-09-19 工具调用展示 **Rev.2** 已实现并集成：内容区改为**固定 16 个代码行高的滚动窗口**（原生滚动，不做虚拟化）、**shiki 按需高亮**（首屏 `index-*.js` 零 shiki 引用，首屏仅增约 4.7 kB）、Memoh 三项视觉（diff 行背景带 + 左指示条、行号 gutter、命令 `$` 前缀 + 错误红色）、**去除全部「展开全部 / 收起」**，且 Chat 正文的 ``` 代码块一并接入同一高亮内核。经**三轮独立 review**（第三轮结论「可合入」）：第二轮抓到 F1 修复引入的 blocking 回归（红色规则被同特异性的灰色规则覆盖 → “泄漏没了，红色也没了”），第三轮确认闭合，并为此新增了**能计算 CSS 级联胜者**的断言。集成态 `npm ci` / `typecheck` / **309 tests** / `build` 全绿；真实浏览器验收仍未做。详见 [`tool-call-detail-ui-plan.md`](./tool-call-detail-ui-plan.md) §11 与 `.agents/tasks/20260918-tool-views-rev2-batch.md`。
