...
| [`tool-call-detail-ui-plan.md`](./tool-call-detail-ui-plan.md) | 方案完成、未获实施授权 | 工具调用**展开区**的按工具定制视图（第一层 bash/read/edit+write/ffgrep+fffind/web_search/web_fetch，第二层 todo/ask_user_question）、`details` 白名单投影、真实数据核对、宿主文件读取边界 |
