...
| 编号 | 决策 | 推荐 |
| --- | --- | --- |
| D1 | 分界线位置：`firstKeptEntryId` 之前（语义边界） vs compaction entry 位置（写入时刻） | **语义边界** |
| D2 | 被摘要的历史消息：保留显示 + 横线提示 vs 隐藏 | **保留显示** |
| D3 | todo 面板位置：composer 上方可折叠条 vs Chat 顶部固定条 vs 侧栏 | **composer 上方可折叠条** |
| D4 | 是否做 P3 一键回答（按键注入） | **先只做 P2**，P3 以后单独决定 |
| D5 | 排期：等当前 Worker 集成后再开新 Worker vs 立即串行开始 | **等集成后再开** |
| D6 | 压缩进行中的实时提示（订阅 `compaction_start/end`） | **暂不做**，先把历史分界做对 |
