...
用户授权后，已把这套输出节奏写进**全局 Pi 上下文文件** `~/.pi/agent/AGENTS.md`，新增 `## 输出节奏（多步任务）` 一节（5 条）：开工前简述打算、过程中主动给阶段性文字且不停下等确认、不报流水账、收尾说清验证与限制、用户要求优先。原文 4 条全局要求未动。

选择全局文件而非项目 `AGENTS.md` 的原因：上一条已说明，Pane 中的 agent 只读它自己的全局与项目上下文文件，不读 Herzi 仓库规则。生效范围是所有 Pi 会话；新会话自动生效，已运行中的会话需重载或重启。

### 5.3 实测验证（2026-09-17）

用一个新开的 Herdr Pane + 新 Pi 会话验证（cwd 为 `/tmp/pi-cadence-test`，无项目 `AGENTS.md`，因此只受全局规则影响；任务为“比较 4 个 coding agent CLI 的会话历史存储设计，需真实检索并给出来源”）。

结果（对 session JSONL 做 part 类型统计，与之前的方法一致）：

| 指标 | 本次测试 | 之前那个会话（未启用规则） |
| --- | --- | --- |
| turn 数 | 1 | 29 |
| 组内包含 text 的 turn | **1 / 1** | 1 / 29 |
| 组内 part 构成 | reasoning 11 / **text 7** / tool-call 38 | reasoning 272 / **text 1** / tool-call 555 |
| 全部 text part | 9（1 条用户 + 8 条 assistant） | — |

7 条中途文本实际内容都是 55–116 字的阶段更新，例如「我先说明思路：分项目检索官方文档与源码仓库……」、「初步命中一批官方文档和源码，现在直接抓取一手来源」、「Codex 的 GitHub 页没返回正文，改用 raw 源码」——即“意图说明 / 阶段结果 / 换方案”，与规则描述一致，且都不是逐个工具报账。最后一段 8601 字为完整报告，仍在组外正常显示。

同时核对 Herzi 侧：`GET /api/panes/<pane>/chat` 对该 Pane 的投影为 `text 9 / reasoning 11 / tool-call 38`，与 session 统计一致，说明展示链路无需改动即能拿到这些中途文本。
...
- 全局规则的效果已在一次真实测试中得到正面证据（见 §5.3），但样本只有 1 个 turn、1 个任务类型；尚未观察短任务是否被过度啰嗦、长时间对话中 token 增量多少。
- 测试 Pane（`wW:p5`，agent `pi_cadence`）与 `/tmp/pi-cadence-test` 在报告时仍保留，供用户自己在 Herzi 里查看效果；未清理。
